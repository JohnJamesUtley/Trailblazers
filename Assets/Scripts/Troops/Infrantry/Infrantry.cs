...
public class Infrantry : Troop
{
    ...
    public override GameObject SpawnDead() {
        GameObject dead = GameObject.Instantiate(Resources.Load<GameObject>("InfrantryDead"));
        ...
        dead.transform.GetChild(1).GetComponent<Orderer>().Staticize();
        return dead;
    }
    void Update() {
        UpdateTroop();
    ...
    public void SetShield(bool newShield) {
        GetComponent<Animator>().SetBool("Shield", newShield);
        shield = newShield;
        if (newShield) {
            transform.GetChild(0).GetChild(0).GetComponent<Orderer>().adjustment = 4;
        } else {
            transform.GetChild(0).GetChild(0).GetComponent<Orderer>().adjustment = 0;
        }
    }
    void Fight() {
            ...
            foreach (Transform e in enemies) {
                if (InBox(transform.position, f.position, e.position)) {
                    if (!e.GetComponent<Troop>().immune) {
                        ...
                        e.GetComponent<Troop>().health -= ((1 / fightPerSec) / 5);
                    }
                }
            }
        }
